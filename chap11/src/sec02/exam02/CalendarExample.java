...
public class CalendarExample {
	...
	public static void main(String[] args) {
		...
		//get메소드가 리턴하는건 다 int값
		int amPm = now.get(Calendar.AM_PM); // 오전: 0, 오후: 1
		String strAmPm = null;
		
		if (amPm == Calendar.AM) { // Calendar.AM  얘도 0인데 가독성을 위해 상수로 쓰는거임
			strAmPm = "오전";
		} else {
			strAmPm = "오후";
		}
		...
		String strAmPm2 = hour24 < 12 ? "오전" : "오후"; // 삼항연산자
		...
		// is가 붙었냐는건 그 전과 같냐라는거니까 , 보통 Boolean 타입이라고 하심
		// 날짜 비교
		LocalDate today = LocalDate.now();
		LocalDate targetDate = LocalDate.of(2025, 12, 25); // 2025년 12월 25일
		
		if (today.isBefore(targetDate)) { // 직관적으로 오늘이, 전인지 , 크리스마스 이렇게 잘 읽힘
