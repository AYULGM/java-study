...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		// 해당 경로에 실제 파일이나 폴더가 있는지 검사 (이미 만들어졌을수도 있으니)
		// 존재하지 않으면 생성
		// 폴더 생성
		if (!dir.exists())
			dir.mkdirs();
...
//		if (!dirTest.exists()) dirTest.mkdir(); // 경로상 맨뒤 폴더만 생성(전제조건: 앞에 상위 폴더가 있어야함)
		...
		File parentDir = dir.getParentFile(); // parentDir.(기능) (부모 디렉터리에서 파일조작등을 해야할경우)
		...
		if (temp.exists() && temp.isDirectory()) { // 존재하고 , 폴더여야 그 하위디렉토리를 가져올수있음(파일이면 isFile()로 해야겠지..)
			...
			// 날짜 포맷 지정 (우리나라 형식으로 만들어야지 보기 편하니)
...
//				System.out.println(file.lastModified()); // 확인용
				// (참고)
				// UNIX 시간(=에포크 시간): 1970년 1월 1일 0시 0분 0초
