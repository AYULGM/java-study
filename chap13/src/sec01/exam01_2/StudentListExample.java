...
public class StudentListExample {
	...
	public static void main(String[] args) {
		...
		// 해당 객체 존재 여부를 판단하는 기준은 equals() 재정의 안하면 == 주소값비교를 해버림
		// equals() 메소드를 재정의(오버라이딩) 해야 동등 객체로 판단
		if (studentList.indexOf(new Student("김재현", 30)) != -1) {// new 연산자로 새롭게 만들어서 false
			System.out.println("indexOf: 있다.");
		} else {
			System.out.println("indexOf: 없다.");
		}

		if (studentList.contains(new Student("김재현", 30))) { // 이것도 재정의 안되어있어서 false
