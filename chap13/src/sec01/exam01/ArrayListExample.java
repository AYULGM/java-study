...
//		List<String> list = new ArrayList<String>(); // ArrayList<E> 말고 List<E>라고 쓰는이유는 다형성때문
		List<String> list = new ArrayList<>(); // 생략하면 왼쪽에 지정된 타입을 따라 감 (타입 추론) 
		
		// 객체 추가
//		list.add(true); // 타입이 맞지않음, 제네릭 타입으로 명시된 타입의 데이터만 추가 가능
		...
		// 특정 인덱스의 객체 가져오기
		String skill = list.get(2); // 꺼내올 객체가 문자열이니 String타입으로 만듦
		System.out.println("2: " + skill);
		System.out.println();
		
		// 리스트 출력하기
		System.out.println(list); // 출력문에 해시코드가 안나왔단거는 , 재정의된 toString()이다.
		
		// 객체 수정
		// 주어진 인덱스에 저장된 객체를 주어진 객체로 바꿈
		list.set(4, "MyBatis"); // 객체가 바뀌는게 아니고 인덱스4가 참조하는 주소가 바뀔뿐
		...
		if (!list.isEmpty()) { // 컬렉션이 비어있는지 조사
			// 주어진 객체가 저장되어 있는지 조사
			System.out.println("JAVA 있니? " + list.contains("Java")); // contains의 리턴타입이 Boolean
			System.out.println("어디에 있니? " + list.indexOf("Java"));
			...
			System.out.println("어디에 있니? " + list.indexOf("Database")); // 못찾으면 -1
		}
		System.out.println();
		...
		// 오름차순
		Collections.sort(strList); // String 클래스 안에 CompareTo 메소드가 있기때문에 오름차순 가능
		System.out.println(strList);
		...
		numList.add(11); // Auto Boxing , Integer객체에 int타입객체가 들어감
		numList.add(5);
		numList.add(1);
		numList.add(9);
		numList.add(3);
