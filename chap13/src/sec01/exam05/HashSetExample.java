...
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		set.remove(new Member("김재현", 30)); //중복된것도 해시,이퀄스 재정의안해서 삭제도 안됨
		
		System.out.println(set); // 중복 저장이 안되려면 해시코드,이퀄스 리턴값재정의해줘야됨
		...
		// 3. 출력(향상된 for문 = for-each문)
		for (Member A : setList) {
		...
		Set<Member> setMem = new HashSet<Member>(memList);
		List<Member> dedupList = new ArrayList<Member>(setMem); // 다시 List타입으로
