...
//	    for each문(향상된 반복문)으로 모든 구성원 소개글 출력 및
...
//			if (member instanceof Student) { //쉽게 Student가 부모 member가 자식이라는뜻
//                ((Student)member).study(); // 이것보단 아래코드가 더 깔끔함
			if (member instanceof Student student) { // 타입체크하면서 타입변환까지 한번에
				student.study();
